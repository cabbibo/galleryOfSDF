...
float3 modit(float3 x, float y)
{
  return x - y * floor(x/y);
            ...
            float2 map( float3 pos ){
                ...
                float2 sphere2 = float2( sdSphere( fPos , .3 + _Parameter3 * .6 ),4);
                
                sphere2 = smoothU( sphere2 ,  float2( sdSphere( fPos2, .3 + _Parameter3 * .6 ),4), .1);
                sphere2.x *= -1;


                float2 final = smoothS( cubes, sphere , .1);
                
                final = hardU( final , cubes2 );
                final = smoothS( sphere2,final  ,.3 );
                final = smoothU( final , float2(-sdSphere(pos,1.7),0.4),.8);
